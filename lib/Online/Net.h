...
namespace net
{
	...
	inline bool InitializeSockets()
	{
		#if PLATFORM == PLATFORM_WINDOWS
	    WSADATA WsaData;
		return WSAStartup( MAKEWORD(2,2), &WsaData ) != SOCKET_ERROR;//NO_ERROR;
		#else
		return true;
		#endif
	}
	...
	public:
		...
		bool SendPacket( const unsigned char data[], int size )
		{
			...
			unsigned char* packet = (unsigned char*) malloc (sizeof (char)*(size+4));
			...
			bool result = socket.Send( address, packet, size + 4 );
			free (packet);
			return result;
		}
